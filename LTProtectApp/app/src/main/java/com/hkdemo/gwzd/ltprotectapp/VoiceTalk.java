...
public class VoiceTalk {
	...
	public static int startVoiceTalk(int iUserID)//only support G711A/U and G722, but G722 Non publication function
	{
		...
		{
			Log.e("获取报警视频","NET_DVR_GetCurrentAudioCompress failed, error:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
			return -1;
		}
		...
		else if(compressAud.byAudioEncType == 0) //G722 
		{
			...
			AudioParam.nBitRate = AudioBitRate.AUDIO_BITRATE_16K;
            Log.i("获取报警视频","G722 模式");
		}
		else
		{
			Log.e("获取报警视频","the device audio type is not support by AudioEngineSDK for android ,type:" + compressAud.byAudioEncType);
			return -1;
		}
		//start AudioEngine
		if(!startAudioEngine(AudioParam))
		{
		    Log.e("获取报警视频","startAudioEngine失败");
			return -1;
		}
		//start HCNetSDK
		if (TalkCbf==null)
		...
		{
			stopVoiceTalk();
			Log.e("获取报警视频","NET_DVR_StartVoiceCom_MR_V30 failed,error:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
		}
	...
	private static boolean startAudioEngine(AudioCodecParam AudioParam)
	{
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","audio engine open failed, error:" + iRet);
			return false;
		}
		//set parameter
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","audio.setAudioParam PARAM_MODE_PLAY failed, error:" + iRet);
			audio.close();
			return false;
		}
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","audio.setAudioParam PARAM_MODE_RECORDE failed, error:" + iRet);
			audio.close();
			return false;
		}
		//set callback
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","audio.setAudioCallBack RECORDE_DATA_CALLBACK failed, error:" + iRet);
			audio.close();
			return false;
		}
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","audio.startPlay failed, error:" + iRet);
			audio.close();
			return false;
		}
		...
		if(iRet != 0)
		{
			Log.e("获取报警视频","开启录音失败, error:" + iRet);
			audio.stopPlay();
			audio.close();
			return false;
		}
